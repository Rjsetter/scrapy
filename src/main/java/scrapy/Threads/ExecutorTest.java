...
public class ExecutorTest {
    ...
    public static void main(String[] args) {
        ...
        for (int j=3;j<4;j++) {
            //按类型查询数据库中得地址
            List<String> uidList = userMapperImp.queryAllUidByType("动漫");
            ...
            int index = 0;//IP地址
            for (int i = 0; i < len; i++) {
                logger.error("正在爬取类型为"+types.get(j)+",Uid为：" + uidList.get(i) + "的关注信息！");
                if (i % 3 == 0) {
                    ++index;
                ...
                pool.execute(new ExcuteTask2(uidList.get(i), ip, port, type,ips));
            }
    ...
    static class ExcuteTask2 implements Runnable {
        ...
        public ExcuteTask2(String taskName, String ip, int port, String type,List<IPBean> ips) {
        ...
        public void run() {
            try {
                ...
                spider(taskName, ip, port, type,ips);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
